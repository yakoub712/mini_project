...
$password = $inputData["password"] ?? ""; // ❌ يتم تخزين كلمة المرور كما هي
$id = $inputData["id"] ?? "";
...
if ($stmt->execute()) {
    echo json_encode(["status" => "success", "message" => " تم إنشاء الحساب بنجاح!", "redirect" => "http://localhost/min_p/Authentication Page.html"]);
} else {
